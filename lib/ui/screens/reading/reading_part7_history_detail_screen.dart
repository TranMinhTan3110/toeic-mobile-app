...
import 'reading_part7_history_overview_screen.dart';

class ReadingPart7HistoryDetailScreen extends StatefulWidget {
  final ReadingPart7HistoryModel historyItem;
  const ReadingPart7HistoryDetailScreen({super.key, required this.historyItem});

  @override
  State<ReadingPart7HistoryDetailScreen> createState() => _ReadingPart7HistoryDetailScreenState();
}

class _ReadingPart7HistoryDetailScreenState extends State<ReadingPart7HistoryDetailScreen> {
  ...
  Widget _buildContent(BuildContext context) {
    ...
    final sessionQuestions = <ReadingPart7Question>[];
  ...
  Widget _buildBannerCard(ReadingPart7HistoryModel item) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Text('Reading Part 7', style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 14, fontWeight: FontWeight.w500)),
  ...
  Widget _buildScoreDetailsCard(ReadingPart7HistoryModel item) {
    return Container(
      padding: const EdgeInsets.all(20),
  ...
  Widget _buildWrongQuestionsSection(List<ReadingPart7Question> wrongQuestions, ReadingPart7HistoryModel item) {
    if (wrongQuestions.isEmpty) {
      return Container(padding: const EdgeInsets.all(24), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(16), boxShadow: const [BoxShadow(color: AppColors.shadow, blurRadius: 8, offset: Offset(0, 2))]), child: const Column(children: [Icon(Icons.check_circle_outline_rounded, color: AppColors.green, size: 48), SizedBox(height: 12), Text('Tuyệt vời! Không có câu hỏi sai', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: AppColors.textPrimary)), SizedBox(height: 4), Text('Bạn đã trả lời đúng 100% các câu hỏi!', style: TextStyle(fontSize: 13, color: AppColors.textSecondary))]));
    }
  ...
  Widget _buildWrongQuestionCard(ReadingPart7Question q, ReadingPart7HistoryModel item) {
    final selectedAns = item.selectedAnswers[q.id];
    final correctAns = q.correctAnswer;

    return Container(margin: const EdgeInsets.only(bottom: 16), padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.divider), boxShadow: const [BoxShadow(color: AppColors.shadow, blurRadius: 6, offset: Offset(0, 2))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(q.prompt, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: AppColors.textPrimary)),
      const SizedBox(height: 12),
  ...
  Widget _buildExplanationSection(ReadingPart7Question q) {
    final explanation = q.explanation ?? q.grammarExplanation ?? '';
    final explanationVi = q.explanationVi ?? q.translation ?? '';
    return Container(width: double.infinity, margin: const EdgeInsets.only(top: 12), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: AppColors.primary, borderRadius: BorderRadius.circular(12)), child: Column(children: [
  ...
  Widget _buildBottomControls(BuildContext context, List<ReadingPart7Question> sessionQuestions) {
    return Container(padding: const EdgeInsets.fromLTRB(16, 12, 16, 24), decoration: BoxDecoration(color: AppColors.surface, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 10, offset: const Offset(0, -4))]), child: Row(children: [
      Expanded(child: ElevatedButton.icon(onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (_) => ReadingPart7HistoryOverviewScreen(historyItem: widget.historyItem, sessionQuestions: sessionQuestions))); }, icon: const Icon(Icons.assignment_rounded, color: Colors.white, size: 20), label: const Text('Chi tiết', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: 0.5)), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 3))), const SizedBox(width: 12), Expanded(child: ElevatedButton.icon(onPressed: () { Navigator.of(context).pop(); }, icon: const Icon(Icons.replay_rounded, color: Colors.white, size: 20), label: const Text('Làm lại', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: 0.5)), style: ElevatedButton.styleFrom(backgroundColor: AppColors.green, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 3))) ]));
  }
}
