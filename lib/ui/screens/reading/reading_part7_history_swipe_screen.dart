                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    if (question.passage.isNotEmpty)
                      Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), boxShadow: const [BoxShadow(color: AppColors.shadow, blurRadius: 8, offset: Offset(0, 2))]), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: AppColors.surface, borderRadius: BorderRadius.circular(8)), child: Text(question.prompt, style: TextStyle(fontSize: _fontSize + 2, fontWeight: FontWeight.w600, color: AppColors.textPrimary, height: 1.6))),
                      ])),
                    if (question.passage.isNotEmpty)
                    ...
                    Column(children: List.generate(question.options.length, (oi) {
                      ...
                      return Container(
                        ...
                        child: Row(children: [
                          Container(width: 32, height: 32, decoration: BoxDecoration(color: badgeColor, shape: BoxShape.circle), child: Center(child: Text(optionKey, style: TextStyle(color: badgeTextColor, fontWeight: FontWeight.bold, fontSize: 14)))),
                          const SizedBox(width: 14),
                          Expanded(child: Text(question.options[oi], style: TextStyle(fontSize: _fontSize, color: AppColors.textPrimary, height: 1.4))),
                        ]),
                      );
                    })),
                    const SizedBox(height: 20),
                    Container(
                      ...
                      decoration: BoxDecoration(color: isCorrect ? AppColors.answerCorrect.withOpacity(0.12) : AppColors.answerWrong.withOpacity(0.12), borderRadius: BorderRadius.circular(8), border: Border.all(color: isCorrect ? AppColors.success : AppColors.error)),
                      child: Row(children: [Icon(isCorrect ? Icons.check_circle : Icons.cancel_rounded, color: isCorrect ? AppColors.success : AppColors.error, size: 20), const SizedBox(width: 10), Text(isCorrect ? 'Đúng' : 'Sai', style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: isCorrect ? AppColors.success : AppColors.error)), ]),
                    ),
                  ]),
                );
              },
