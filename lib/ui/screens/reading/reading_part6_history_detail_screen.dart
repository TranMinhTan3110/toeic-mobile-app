...
class ReadingPart6HistoryDetailScreen extends StatefulWidget {
  ...
  @override
  State<ReadingPart6HistoryDetailScreen> createState() => _ReadingPart6HistoryDetailScreenState();
}

class _ReadingPart6HistoryDetailScreenState extends State<ReadingPart6HistoryDetailScreen> {
  ...
  Widget _buildBannerCard(ReadingPart6HistoryModel item) {
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Text('Reading Part 6', style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 14, fontWeight: FontWeight.w500)),
  ...
  Widget _buildWrongQuestionsSection(List<ReadingPart6Question> wrongQuestions, ReadingPart6HistoryModel item) {
  ...
  Widget _buildWrongQuestionCard(ReadingPart6Question q, ReadingPart6HistoryModel item) {
  ...
  Widget _buildExplanationSection(ReadingPart6Question q) {
  ...
  Widget _buildBottomControls(BuildContext context, List<ReadingPart6Question> sessionQuestions) {
    return Container(padding: const EdgeInsets.fromLTRB(16, 12, 16, 24), decoration: BoxDecoration(color: AppColors.surface, boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.06), blurRadius: 10, offset: const Offset(0, -4))]), child: Row(children: [
      Expanded(child: ElevatedButton.icon(onPressed: () { Navigator.push(context, MaterialPageRoute(builder: (_) => ReadingPart6HistoryOverviewScreen(historyItem: widget.historyItem, sessionQuestions: sessionQuestions))); }, icon: const Icon(Icons.assignment_rounded, color: Colors.white, size: 20), label: const Text('Chi tiết', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: 0.5)), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 3))), const SizedBox(width: 12), Expanded(child: ElevatedButton.icon(onPressed: () { Navigator.of(context).pop(); }, icon: const Icon(Icons.replay_rounded, color: Colors.white, size: 20), label: const Text('Làm lại', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, letterSpacing: 0.5)), style: ElevatedButton.styleFrom(backgroundColor: AppColors.green, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 3))) ]));
  }
}
